    ...
    int countTrapezoidsImpl(const std::vector<std::vector<int>>& points,
        ...
        for (int i = 1; i < (int)points.size(); ++i) {
            for (int j = 0; j < i; ++j) {
                ...
                // Fix a canonical direction
                if (dx < 0 || (dx == 0 && dy < 0)) {
                    dx = -dx;
                    dy = -dy;
                }

                // Encode direction as a single integer key
                int key = 10000 * dx + dy;

                // Identify the parallel line via "height"
        ...
        // In the non-normalized case we counted each parallelogram twice
        if (!normalize) {
            result >>= 1;
        }
        return result;
